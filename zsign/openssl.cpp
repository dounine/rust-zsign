...
bool _GenerateCMS(X509 *scert, EVP_PKEY *spkey, const string &strCDHashData, const string &strCDHashesPlist,
    ...
    X509 *ocert1 = PEM_read_bio_X509(bother1, nullptr, nullptr, nullptr);
    X509 *ocert2 = PEM_read_bio_X509(bother2, nullptr, nullptr, nullptr);
    if (!ocert1 || !ocert2) {
        return CMSError();
    }
    ...
    CMS_ContentInfo *cms = CMS_sign(nullptr, nullptr, otherCerts, nullptr, nFlags);
    if (!cms) {
        return CMSError();
    }
...
//    CMS_add1_signer(cms, NULL, NULL, EVP_sha1(), nFlags);
    ...
    string sha256;
    char buf[16] = {0};
    for (char i: strAltnateCodeDirectorySlot256) {
    ...
    if (!CMS_final(cms, in, nullptr, nFlags)) {
        return CMSError();
    }
...
bool GenerateCMS(const string &strSignerCertData, const string &strSignerPKeyData, const string &strCDHashData,
    ...
    X509 *scert = PEM_read_bio_X509(bcert, nullptr, nullptr, nullptr);
    EVP_PKEY *spkey = PEM_read_bio_PrivateKey(bpkey, nullptr, nullptr, nullptr);
    if (!scert || !spkey) {
        return CMSError();
    }
...
bool GetCertSubjectCN(const string &strCertData, string &strSubjectCN) {
    ...
    X509 *cert = PEM_read_bio_X509(bcert, nullptr, nullptr, nullptr);
    if (!cert) {
        return CMSError();
    }
...
bool GetCertInfo(X509 *cert, JValue &jvCertInfo) {
    ...
    if (asn1_i) {
        BIGNUM *bignum = ASN1_INTEGER_to_BN(asn1_i, nullptr);
        if (bignum) {
    ...
    string strIssuer = X509_NAME_oneline(X509_get_issuer_name(cert), nullptr, 0);
    string strSubject = X509_NAME_oneline(X509_get_subject_name(cert), nullptr, 0);
...
bool ZSignAsset::Init(const string &strSignerCertFile, const string &strSignerPKeyFile, const string &strProvisionFile,
    ...
    if (nullptr == x509Cert && !strSignerCertFile.empty()) {
        BIO *bioCert = BIO_new_file(strSignerCertFile.c_str(), "r");
        if (nullptr != bioCert) {
            x509Cert = PEM_read_bio_X509(bioCert, nullptr, nullptr, nullptr);
            if (nullptr == x509Cert) {
                BIO_reset(bioCert);
                x509Cert = d2i_X509_bio(bioCert, nullptr);
            }
            BIO_free(bioCert);
        }
    }
